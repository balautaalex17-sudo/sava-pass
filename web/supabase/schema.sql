...
create index team_members_active_sort_idx on public.team_members (active, sort);
...
create index projects_published_sort_idx on public.projects (published, sort);
...
create index sponsors_active_sort_idx on public.sponsors (active, sort);
...
-- team / projects / sponsors: public reads visible rows, staff reads all, admin writes
create policy team_members_read  on public.team_members for select to public using ((active = true) or is_staff());
create policy team_members_write on public.team_members for all    to public using (is_admin()) with check (is_admin());
create policy projects_read  on public.projects for select to public using ((published = true) or is_staff());
create policy projects_write on public.projects for all    to public using (is_admin()) with check (is_admin());
create policy sponsors_read  on public.sponsors for select to public using ((active = true) or is_staff());
create policy sponsors_write on public.sponsors for all    to public using (is_admin()) with check (is_admin());

-- site_content: public reads everything, admin writes
create policy site_content_read  on public.site_content for select to public using (true);
create policy site_content_write on public.site_content for all    to public using (is_admin()) with check (is_admin());

-- contact_messages: anon insert, staff read, admin marks handled (mirrors membership_applications)
create policy contact_messages_insert       on public.contact_messages for insert to public        with check (true);
create policy contact_messages_staff_read   on public.contact_messages for select to authenticated using (is_staff());
create policy contact_messages_admin_update on public.contact_messages for update to authenticated using (is_admin()) with check (is_admin());

-- Storage: public 'media' bucket — anyone reads via public URL, only staff write.
