...
export default async function GalleryPage({ searchParams }: { searchParams: Promise<{ page?: string; drive?: string }> }) {
  ...
  return (
    <main className={styles.page}>
      <div className={styles.shell}>
        <Link className={styles.back} href={viewer.membershipStatus === "active" ? "/membru" : "/conta"}><ArrowLeft size={16} /> Înapoi la cont</Link>
        ...
        <nav className={styles.pagination} aria-label="Pagini galerie">
          {page > 1 && <Link className={styles.secondary} href={`/conta/galerie?page=${page - 1}`}>Pagina anterioară</Link>}
          {(page > 1 || hasNext) && <span>Pagina {page}</span>}
          {hasNext && <Link className={styles.secondary} href={`/conta/galerie?page=${page + 1}`}>Mai multe poze</Link>}
        </nav>
      </div>
    </main>
  );
