...
export function ArchiveEventEditor({ event, mediaAssets }: { event: EventRecord; mediaAssets: MediaOption[] }) {
  const [saveState, saveAction, savePending] = useActionState(saveArchiveEvent, {} as ArchiveEventActionState);
  const [archiveState, archiveAction, archivePending] = useActionState(archiveImportedEvent, {} as ArchiveEventActionState);
  const [coverUrl, setCoverUrl] = useState(event.coverImage.src);
  const [published, setPublished] = useState(event.publishingStatus === "published");
  const error = saveState.error || archiveState.error;
  const message = saveState.message || archiveState.message;

  return (
    <div className="archive-event-editor">
      <div className="archive-event-toolbar" role="region" aria-label="Acțiuni eveniment">
        <div className="archive-event-toolbar__copy">
          <strong>{published ? "Eveniment public" : "Eveniment arhivat"}</strong>
          <span>
            {published
              ? "Arhivarea îl ascunde de pe site, dar păstrează sursa și modificările."
              : "Bifează „Vizibil pe site” și salvează pentru a-l republica."}
          </span>
        </div>

        <div className="archive-event-toolbar__actions">
          {published && (
            <form action={archiveAction}>
              <input type="hidden" name="slug" value={event.slug} />
              <button type="submit" className="dash-button dash-button--secondary archive-event-toolbar__archive" disabled={archivePending || savePending}>
                <Archive size={17} aria-hidden="true" />
                {archivePending ? "Se arhivează…" : "Arhivează"}
              </button>
            </form>
          )}
          <button type="submit" form="archive-event-editor-form" className="dash-button" disabled={savePending || archivePending}>
            <Save size={17} aria-hidden="true" />
            {savePending ? "Se salvează…" : "Salvează modificările"}
          </button>
        </div>

        {(error || message) && (
          <p className={`dash-form-message archive-event-toolbar__feedback ${error ? "dash-form-message--error" : "dash-form-message--success"}`} role={error ? "alert" : "status"}>
            {error || message}
          </p>
        )}
      </div>

      <form id="archive-event-editor-form" action={saveAction} className="archive-event-editor__form">
        <input type="hidden" name="slug" value={event.slug} />
        ...
        <section className="dash-card" style={sectionStyle}>
          <div style={sectionHeadingStyle}>
            ...
            <label style={toggleStyle}>
              <input name="published" type="checkbox" checked={published} onChange={(inputEvent) => setPublished(inputEvent.target.checked)} />
              Vizibil pe site
            </label>
          </div>
        ...
        <div className="archive-event-save-row">
          <button type="submit" className="dash-button" disabled={savePending || archivePending}>
            <Save size={17} aria-hidden="true" />
            {savePending ? "Se salvează…" : "Salvează modificările"}
          </button>
        </div>
