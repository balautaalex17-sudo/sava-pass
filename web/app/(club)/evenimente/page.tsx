...
function featuredLabel(event: EventRecord) {
  ...
  if (event.eventStatus === "upcoming") return "Urmează";
  return "Cel mai recent eveniment";
}
...
export default async function EventsPage() {
  const [ticketingRows, importedEvents] = await Promise.all([
  ...
  const events = [
    ...ticketingEvents,
    ...importedEvents.filter((event) => !ticketingSlugs.has(event.slug)),
  ...
  const featured = events
    ...
    || events[0]
    || null;
  const charitableCount = events.filter((event) => event.charitableCause).length;
  const causeOrganizations = CAUSE_ORGANIZATIONS.map((cause) => ({
    ...cause,
    editions: events.filter((event) => event.charitableCause?.includes(cause.match)).length,
