...
const eventSchema = z.object({
  ...
  charitable_cause: z.string().trim().optional(),
...
export async function upsertEvent(_prev: EventActionState, form: FormData): Promise<EventActionState> {
  ...
  const payload = {
    ...
    price_bani: Math.min(...(ticketTypes.data.some((type) => type.status !== "hidden")
      ? ticketTypes.data.filter((type) => type.status !== "hidden")
      : ticketTypes.data).map((type) => type.price_ron)) * 100,
    capacity: parsed.data.capacity,
    about: parsed.data.about || null,
    charitable_cause: parsed.data.charitable_cause || null,
