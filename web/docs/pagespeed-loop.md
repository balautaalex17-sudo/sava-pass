...
- **2026-06-25 — baseline** (from pagespeed.web.dev, mobile): FCP 2.1s / LCP 3.2s / Speed Index 4.6s RED; TBT 20ms / CLS 0 GREEN. LCP element = `/imersiv/church.webp` loaded `loading="lazy"` (1380ms load delay). Anonymous PSI API quota was exhausted (429) at build time — re-run `psi-gate.mjs` with a `PSI_API_KEY` for the live verdict.
