...
- **2026-06-25 — round 2 deployed** (`dpl_…935hw1c44`). Round-1's live PSI (mobile) read FCP 2.1s / LCP 3.8s / Speed Index 3.7s — **all orange (50-89), out of red**; TBT/CLS green; render-blocking CSS gone and the lazy-LCP flag cleared. Remaining critical-path bottleneck = **5 Manrope woff2** (~460ms each). Applied: **Manrope `preload:false`** (all fonts off the critical path — verified **0 font preloads** in the deployed HTML, the only mobile preload is now the LCP image) + **`ImmersiveEngine` deferred to first-interaction/idle** (keeps its script-eval out of the LCP window; guard stays green). Left unfixed: unused-JS (49 KiB) + legacy-JS (14 KiB) — both **"Nu se adaugă la scor"** (non-scored, won't change green/red). Re-run pagespeed.web.dev for the verdict.
